...
Le contrôleur utilise explicitement squash avec une précondition sur la tête. Avant mutation, les deux préflights doivent confirmer le même dépôt, des réglages de fusion connus, squash autorisé et la suppression automatique des branches désactivée. Ces réglages font partie de la comparaison de stabilité ; leur absence ou une dérive interdit le merge. Le contrôleur ne modifie jamais la configuration GitHub : une modification nécessaire exige une autorisation applicable et une vérification séparée, sans affaiblir les protections. L'absence de `--delete-branch` ne neutralise pas le réglage global de GitHub.

Après la mutation, relire la PR fusionnée, le commit résultant et sa présence dans la cible distante. Avec squash, le SHA résultant diffère normalement de la tête approuvée : vérifier aussi l'identité de l'arbre intégré et conserver les deux identités. Un retour ambigu est une opération à réconcilier, pas un échec à rejouer aveuglément. Une PR seulement fermée, une tête différente de celle approuvée ou une ancienne capture ne prouvent pas cette intégration. Un reçu local relate l'observation distante datée ; sa relecture hors ligne ne devient pas une nouvelle vérification réseau.
...
La porte de clôture vérifie aussi les pièces locales ignorées du rapport de validation : leurs octets actuels doivent correspondre aux entrées du manifeste. Modifier un dossier, un checkpoint ou un accord impose donc une nouvelle validation. Les snapshots historiques de clôture doivent être cohérents entre eux ; ils ne doivent pas être identiques au registre courant, qui doit pouvoir passer de working à ready. Le contrôleur requalifie séparément l'état courant et le résumé final est régénéré après cette transition.
