...
Avant de rédiger ou réviser un prompt Claude Design, lire [le contrôle de remise](../../../docs/design/claude-design-workflow.md) et les sources officielles actuelles. Distinguer création native, import, écran et itération ; ne pas imposer des assets préexistants à la création native. Conserver prompt et contexte exacts dans un dossier, obtenir une revue indépendante puis exécuter `node scripts/design-prompt.mjs check --dossier <dossier.json>` juste avant remise. Cela vaut aussi pour les compléments et prompts ignorés ; un ancien reçu, un audit CI ou une réponse improvisée en chat ne satisfait pas ce contrôle. Une impossibilité de vérification suspend toute remise du prompt, pas les autres préparations autorisées.
