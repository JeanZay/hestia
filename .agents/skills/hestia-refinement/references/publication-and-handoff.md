...
Appliquer d'abord le [workflow produit](../../../../docs/product-workflow.md) : le plan d'US est réservé au besoin engagé maintenant, après cadrage complet et GO du brief. Pour publier seulement un besoin large ou son brief sans engagement, préparer le delta/commentaire exact et son reçu sous un contrat de publication borné ; ne pas forcer le format V1 à créer des US artificielles. Le circuit détaillé ci-dessous s'applique à un véritable plan juste-à-temps.
...
- qualification lifecycle du périmètre engagé, couverture revue indépendamment, engagement toujours actif et contrôle `--action execute` ; le V1 seul n'atteste pas cette maturité ;
